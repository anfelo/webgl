...
export const m4 = {
    ...
    perspective: function (fieldOfViewInRadians: number, aspect: number, near: number, far: number): Matrix4 {
    ...
    vectorMultiply: function (v: Vector4, m: Matrix4): Vector4 {
        let dst = [0.0, 0.0, 0.0, 0.0] as Vector4;
        for (let i = 0; i < 4; ++i) {
            dst[i] = 0.0;
            for (let j = 0; j < 4; ++j) {
                dst[i] += v[j] * m[j * 4 + i];
    ...
    translate: function (m: Matrix4, tx: number, ty: number, tz: number): Matrix4 {
    ...
    inverse: function (m: Matrix4): Matrix4 {
        const m00 = m[0 * 4 + 0];
        const m01 = m[0 * 4 + 1];
        const m02 = m[0 * 4 + 2];
        const m03 = m[0 * 4 + 3];
    ...
    transpose: function (m: Matrix4): Matrix4 {
        return [m[0], m[4], m[8], m[12], m[1], m[5], m[9], m[13], m[2], m[6], m[10], m[14], m[3], m[7], m[11], m[15]];
    }
};

export const m3 = {
    multiply: function (a: Matrix3, b: Matrix3): Matrix3 {
        const a00 = a[0 * 3 + 0];
        const a01 = a[0 * 3 + 1];
        const a02 = a[0 * 3 + 2];
        const a10 = a[1 * 3 + 0];
    ...
    translation: function (tx: number, ty: number): Matrix3 {
        return [1, 0, 0, 0, 1, 0, tx, ty, 1];
    },

    rotation: function (angleInRadians: number): Matrix3 {
        const c = Math.cos(angleInRadians);
        const s = Math.sin(angleInRadians);
        return [c, -s, 0, s, c, 0, 0, 0, 1];
    },

    scaling: function (sx: number, sy: number): Matrix3 {
        return [sx, 0, 0, 0, sy, 0, 0, 0, 1];
    },

    // This projection matrix is used to convert from pixels to clipspace
    projection: function (width: number, height: number): Matrix3 {
        // Note: This matrix flips the Y axis so that 0 is at the top.
        return [2 / width, 0, 0, 0, -2 / height, 0, -1, 1, 1];
    },

    translate: function (m: Matrix3, tx: number, ty: number): Matrix3 {
        return m3.multiply(m, m3.translation(tx, ty));
    },

    rotate: function (m: Matrix3, angleInRadians: number): Matrix3 {
        return m3.multiply(m, m3.rotation(angleInRadians));
    },

    scale: function (m: Matrix3, sx: number, sy: number): Matrix3 {
...
/**
 * Normalizes a vector.
 * @param {Vector3 | Float32Array} v vector to normalize
 * @param {Vector3 | Float32Array} dst optional vector3 to store result
 * @return {Vector3 | Float32Array} dst or new Vector3 if not provided
 */
export function normalize(v: Vector3 | Float32Array, dst?: Vector3 | Float32Array): Vector3 | Float32Array {
...
/**
 * Calculated the cross product between 2 vectors
 * @param {Vector3 | Float32Array} a
 * @param {Vector3 | Float32Array} b
 * @returns {Vector3 | Float32Array}
 */
function cross(a: Vector3 | Float32Array, b: Vector3 | Float32Array): Vector3 | Float32Array {
